...
## Generated challenge campaign and progressive hints (2026-09-19)

- Each new facility session now gets a cryptographically shuffled challenge order, so the next weakness is no longer the same fixed sequence.
- Every level exposes a `current_challenge` with a briefing and vulnerable FastAPI excerpt. In Live mode, a dedicated typed PydanticAI agent on `gemini-3.5-flash` rewrites that level's code and story from a trusted server blueprint. The web process never imports arbitrary model code; the server executes a constrained, tested mechanic.
- Generated vulnerable and patched snippets are sent to `vault.challenge_worker` in a network-blocked Modal sandbox. The worker parses and compiles both, rejects imports/file/process/network/dynamic-execution primitives, checks that the patch differs, verifies the exact route/mechanic contract, and ensures no session flag is embedded. Only a passing result gets the `Gemini · Modal verified` badge. Verified live with sandbox `sb-BQR758Qkd7z3bkipLJ8uMY` (8/8 checks).
- When a flag is submitted, the exact generated secure patch is saved as `last_patch` and shown in the UI. The next level is created in the background so the breach animation does not wait on Gemini.
- Hints are now tied to the session's current challenge. Completing a level automatically reveals the next level's first actionable clue. Each additional hint becomes more explicit, ending with an exact request or payload.
...
- Tightened after live generation: accepted Gemini revisions must contain `app.py`, `web/index.html`, and `README.md` in both the vulnerable and patched codebases. The prompt now explicitly requests those files, and the isolated validator enforces them.
- Revision continuity: after a breach, Mr Kak's patched files are saved as the inherited codebase. The next Gemini build receives those secured files as context, so every new feature is added to the previously repaired sandbox rather than generated as an unrelated puzzle.
- Public verification found that a real generated FastAPI revision could be rejected because the validator blocked every import. The validator now permits a minimal static allow-list (`fastapi`, `pydantic`, `typing`, `json`, `hmac`, `hashlib`, `logging`, `base64`) while continuing to reject file, process, network, dynamic-execution, and all other imports. This preserves the sandbox boundary while allowing a credible generated website codebase.
- Verified on the public Modal deployment: Gemini produced the required `app.py`, `web/index.html`, and `README.md`; Modal approved the revision. A real synthetic prompt-injection breach then recorded `Simply → Mr Kak → Simply` at revision 2, preserved the three patched files, and exposed the next level's actionable hint.
